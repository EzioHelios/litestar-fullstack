...
def postgres_image() -> str:
    """Use alpine image for faster startup in tests."""
    return "postgres:16-alpine"


@pytest.fixture(scope="session")
def postgres_service(docker_service: DockerService, postgres_image: str) -> Generator[PostgresService, None, None]:
    """Override default postgres_service with longer timeout for slow environments."""
    import psycopg
    ...
    def check(_service: ServiceContainer) -> bool:
        try:
            with psycopg.connect(
                f"host={_service.host} port={_service.port} user={user} password={password} dbname=postgres",
                connect_timeout=5,
            ) as conn:
